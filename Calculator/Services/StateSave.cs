...
public class StateCommand : IUndoableCommand
{
    ...
    private CalculatorState? _stateBefore;
    private CalculatorState? _stateAfter;
    ...
    public void Execute()
    {
        if (_stateAfter == null)
        {
            _stateBefore = _getState(); 
            _action();                  
            _stateAfter = _getState();  
        }
        else
        {
            _setState(_stateAfter);
        }
    }
