...
public partial class MainWindowViewModel : ObservableObject
{
   ...
   private readonly VolumeConverterService _volumeService = new(); // Додано сервіс об'єму
   ...
   [ObservableProperty] private bool _isVolumeVisible = false; // Додано видимість об'єму
   ...
   partial void OnDisplayChanged(string value)
   {
       ...
       RealTimeVolumeConvert(); // Виклик конвертера об'єму
   ...
   {
       if (IsCurrencyVisible || IsVolumeVisible || IsHackerMode) return;
       ExecuteWithHistory(() =>
       {
           if (IsTextErrorOrEasterEgg()) return;
       ...
       switch (mode)
       {
           ...
           case "Volume":
               ...
               if (AvailableVolumeUnits.Count == 0)
               {
                   ...
                   SelectedVolumeFrom = AvailableVolumeUnits[2]; // Літри за замовчуванням
                   SelectedVolumeTo = AvailableVolumeUnits[0];   // Мілілітри за замовчуванням
               }
