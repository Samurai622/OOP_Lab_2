...
public partial class MainWindowViewModel : ObservableObject
{
    ...
    [ObservableProperty] private ObservableCollection<string> _availableVolumeUnits = new();
    [ObservableProperty] private string? _selectedVolumeFrom;
    [ObservableProperty] private string? _selectedVolumeTo;
    ...
    private bool IsTextErrorOrEasterEgg()
    {
        return Display == "Error" || 
               Display == "Ділення на нуль неможливе" ||
               Display == "Недійсне введення" ||
               Display == "Переповнення" ||
               Display.StartsWith("Помилка 404") || 
    ...
    {
        if (double.IsNaN(result)) return "Недійсне введення";
        if (double.IsInfinity(result)) return "Переповнення";
    ...
    public void Calculate()
    {
        ...
        ExecuteWithHistory(() =>
        {
            ...
            if (_isNewInput && !string.IsNullOrEmpty(_lastOperator) && _lastRightOperand != null)
            {
                ...
                catch (DivideByZeroException) { Display = "Ділення на нуль неможливе"; }
                catch { Display = "Недійсне введення"; }
                return;
            }
    ...
    private void RealTimeVolumeConvert()
    {
        if (!IsVolumeVisible) return;
        if (string.IsNullOrEmpty(SelectedVolumeFrom) || string.IsNullOrEmpty(SelectedVolumeTo)) 
        { 
        ...
        {
            double amount = EvaluateTokens(Tokenize(Display));
            double res = _volumeService.Convert(amount, SelectedVolumeFrom, SelectedVolumeTo);
            
            string shortTo = SelectedVolumeTo.Split(' ').Last().Trim('(', ')');
            Equation = $"= {Math.Round(res, 6).ToString(CultureInfo.InvariantCulture)} {shortTo}";
        }
        catch { Equation = "Помилка вводу"; }
    }

    private void RealTimeLengthConvert()
    {
        if (!IsLengthVisible) return;
        if (string.IsNullOrEmpty(SelectedLengthFrom) || string.IsNullOrEmpty(SelectedLengthTo)) 
        { 
            Equation = "Оберіть одиниці"; 
            return; 
        }
        ...
        {
            double amount = EvaluateTokens(Tokenize(Display));
            double res = _lengthService.Convert(amount, SelectedLengthFrom, SelectedLengthTo);
            
            string shortTo = SelectedLengthTo.Split(' ').Last().Trim('(', ')');
            Equation = $"= {Math.Round(res, 6).ToString(CultureInfo.InvariantCulture)} {shortTo}";
        }
        catch { Equation = "Помилка вводу"; }
    }
    ...
    public async Task SetModeAsync(string mode)
    {
        ...
        switch (mode)
        {
            ...
            case "Volume":
                ...
                IsStandardVisible = true; 
                ModeButtonText = "Об'єм";
                if (AvailableVolumeUnits.Count == 0)
                {
                    AvailableVolumeUnits = new ObservableCollection<string>(_volumeService.GetUnits());
                    SelectedVolumeFrom = AvailableVolumeUnits[2]; 
                    SelectedVolumeTo = AvailableVolumeUnits[0];   
                }
                RealTimeVolumeConvert(); 
                break;
            case "Length":
                ...
                IsStandardVisible = true; 
                ModeButtonText = "Довжина";
                if (AvailableLengthUnits.Count == 0)
                {
                    AvailableLengthUnits = new ObservableCollection<string>(_lengthService.GetUnits());
                    SelectedLengthFrom = AvailableLengthUnits[3]; 
